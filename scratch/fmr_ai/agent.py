...
def main():
    ...
    try:
        while not _STOP:
            ...
            rnti = np.array(list(obs_msg.rnti), dtype=np.uint16)[:num_ues]
            dl_mcs = np.array(list(obs_msg.dl_mcs), dtype=np.uint16)[:num_ues]
            buf_req = np.array(list(obs_msg.buf_req), dtype=np.uint32)[:num_ues]
            ...
            # Match offline FMR "in_action":
            # alpha = a_min + (a_max - a_min) * sigmoid(raw_alpha / temp)
            ...
            if args.act_dim >= args.n_ues + 1:
                ...
                z = raw_alpha / max(1e-6, temp)
                sig = 1.0 / (1.0 + np.exp(-z))
                alpha_next = a_min + (a_max - a_min) * sig

            n_active = max(0, min(num_ues, args.n_ues))
            alloc_active = (
                _fractions_to_int_alloc(pref[:n_active], total_rbg) if n_active > 0 else np.zeros((0,), dtype=np.uint16)
            )
